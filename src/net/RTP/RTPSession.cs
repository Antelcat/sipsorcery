...
public class RTPSession : IMediaSession, IDisposable
{
    ...
    protected readonly List<List<SDPSsrcAttribute>> audioRemoteSDPSsrcAttributes = [];
    protected readonly List<List<SDPSsrcAttribute>> videoRemoteSDPSsrcAttributes = [];
    ...
    /// </summary>
    public MediaStream? PrimaryStream => primaryStream;
    ...
    /// </summary>
    [MemberNotNullWhen(true, nameof(AudioStream))]
    public bool HasAudio => AudioStream?.HasAudio == true;

    /// <summary>
    /// Indicates whether this session is using video.
    /// </summary>
    [MemberNotNullWhen(true, nameof(VideoStream))]
    ...
    {
        get => acceptRtpFromAny;

        set
        {
            acceptRtpFromAny = value;
    ...
    public RTPSession(RtpSessionConfig config, bool acceptRtpFromAny)
    {
        rtpSessionConfig = config;
        this.acceptRtpFromAny = acceptRtpFromAny;
        sdpAnnouncementVersion = 0;
        sdpSessionId = Crypto.GetRandomInt(SDP_SESSIONID_LENGTH).ToString();
    ...
    private SDP GetSessionDescription(List<MediaStream> mediaStreamList, IPAddress connectionAddress)
    {
        ...
        sdp.SessionId = sdpSessionId;
        sdp.AnnouncementVersion = sdpAnnouncementVersion;
        ...
        foreach (var mediaStream in mediaStreamList)
        {
                ...
                mediaStream.LocalTrack.StreamStatus != MediaStreamStatusEnum.Inactive)
            {
                ...
                {
                    rtpPort = primaryStream.GetRTPChannel().RTPPort;
                }
                else
                {
    ...
    protected virtual RTPChannel CreateRtpChannel()
    {
        ...
        int bindPort = (rtpSessionConfig.BindPort == 0) ? 0 : rtpSessionConfig.BindPort + rtpChannelsCount * 2;
        ...
        if (rtcpPkt != null)
        {
            ...
            if (rtcpPkt.Bye != null)
            {
                ...
                else
                {
                    ...
                    if ((primaryStream?.RemoteTrack == null) && (primaryStream?.LocalTrack == null))
                    {
    ...
    private MediaStream? GetMediaStreamRemoteSDPSsrcAttributes(uint ssrc)
    {
        ...
        {
            if (audioRemoteSDPSsrcAttributes[index].Any(ssrcAttributes => ssrcAttributes.SSRC == ssrc))
            {
                found = true;
            }
            if (found)
        ...
        {
            if (videoRemoteSDPSsrcAttributes[index].Any(ssrcAttributes => ssrcAttributes.SSRC == ssrc))
            {
                found = true;
            }
            if (found)
    ...
    private MediaStream? GetMediaStream(RTCPCompoundPacket rtcpPkt)
    {
        ...
        if (receptionReports is { Count: > 0 })
        {
            return receptionReports.Select(recRep => GetMediaStream(recRep.SSRC)).OfType<MediaStream>().FirstOrDefault();
        }
