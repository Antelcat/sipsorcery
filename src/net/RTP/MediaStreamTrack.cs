...
public class MediaStreamTrack
{
    ...
    /// RTCP report.
    /// </summary>
    public Dictionary<uint, SDPSsrcAttribute> SdpSsrc { get; set; } = new();
        ...
        Dictionary<int, RTPHeaderExtension>? headerExtensions = null)
    {
        ...
        {
            foreach (var ssrcAttr in ssrcAttributes.Where(ssrcAttr => !SdpSsrc.ContainsKey(ssrcAttr.SSRC)))
            {
    ...
    public MediaStreamTrack(
        AudioFormat format,
        MediaStreamStatusEnum streamStatus = MediaStreamStatusEnum.SendRecv) :
        this(SDPMediaTypesEnum.audio, false, [new SDPAudioVideoMediaFormat(format)], streamStatus)
    {
    }
    ...
    public MediaStreamTrack(
        VideoFormat format,
        MediaStreamStatusEnum streamStatus = MediaStreamStatusEnum.SendRecv) :
        this(SDPMediaTypesEnum.video, false, [new SDPAudioVideoMediaFormat(format)], streamStatus)
    {
    }
    ...
    private bool RestrictCapabilities(SDPAudioVideoMediaFormat sdpAudioVideoMediaFormat)
    {
        var result = true;
        ...
        if (result)
        {
            Capabilities = [sdpAudioVideoMediaFormat];
        }
        return true;
    }
    ...
    public ushort GetNextSeqNum()
    {
        ...
        do
        {
            ...
            actualSeqNum = Interlocked.CompareExchange(ref seqNum, nextSeqNum, expectedSeqNum);
        }
        while
            (expectedSeqNum !=
