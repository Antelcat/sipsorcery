...
public static class DtlsUtils
{
    ...
    public static TlsAgreementCredentials LoadAgreementCredentials(
        TlsContext context,
        ...
        string keyResource)
    {
        Certificate certificate = LoadCertificateChain(certResources);
        AsymmetricKeyParameter privateKey = LoadPrivateKeyResource(keyResource);
        return LoadEncryptionCredentials(context,
            certificate,
            privateKey);
    }

    public static TlsSignerCredentials LoadSignerCredentials(
        TlsContext context,
        Certificate certificate,
        AsymmetricKeyParameter privateKey)
    {
        ...
        SignatureAndHashAlgorithm signatureAndHashAlgorithm)
    {
        ...
        return LoadSignerCredentials(context,
            certificate,
            privateKey,
            signatureAndHashAlgorithm);
    }
    ...
    public static TlsSignerCredentials? LoadSignerCredentials(
        ...
        string keyResource)
    {
        var certificate = LoadCertificateChain([certResource, "x509-ca.pem"]);
        var privateKey = LoadPrivateKeyResource(keyResource);
        return LoadSignerCredentials(context,
    ...
    {
        return LoadCertificateChain([certificate]);
    }

    public static Certificate LoadCertificateChain(string[] resources)
    {
        var chain = new X509CertificateStructure[resources.Length];
...
#pragma warning restore SYSLIB0028
    ...
    public static X509Certificate2 ConvertBouncyCert(Org.BouncyCastle.X509.X509Certificate bouncyCert, AsymmetricCipherKeyPair keyPair)
    {
        ...
        pkcs12Store.Save(pfxStream, [], new SecureRandom());
        pfxStream.Seek(0, SeekOrigin.Begin);
        var keyedCert = new X509Certificate2(pfxStream.ToArray(), string.Empty, X509KeyStorageFlags.Exportable);
    ...
    public static bool IsHashSupported(string hashAlgorithm)
    {
        ...
        {
            "sha1" or "sha-1" or "sha256" or "sha-256" or "sha384" or "sha-384" or "sha512" or "sha-512" => true,
            _ => false
        };
    }
}